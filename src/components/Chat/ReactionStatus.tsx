  ...
  return (
    <>
      <div
        ...
        className={`absolute top-full flex -translate-y-1/2 cursor-pointer items-center gap-[2px] rounded-lg border border-dark bg-dark-lighten px-2 text-sm ${
          position === 'right'
            ? 'right-8'
            : position === 'left-tab'
            ? 'left-[70px]'
      ...
      {isReactionStatusOpened && (
        <div
          ...
          className="fixed top-0 left-0 z-20 flex h-full w-full animate-fade-in items-center justify-center bg-[#00000080]"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="flex h-96 w-screen max-w-[400px] flex-col rounded-lg bg-dark"
          >
            <div className="flex flex-shrink-0 items-center border-b border-dark-lighten p-4">
              ...
              <div className="flex flex-1 justify-end">
                <button
                  onClick={() => setIsReactionStatusOpened(false)}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-dark-lighten transition duration-300 hover:brightness-125"
                >
